...
<div id="twitter" class="section clearfix<?php echo $bg_cover_class . $parallax_class . $color_scheme_class; ?>">
    <div class="container">
        ...
        if(empty($mts_options['homepage_twitter_api_key']) || empty($mts_options['homepage_twitter_api_secret']) || empty($mts_options['homepage_twitter_access_token']) || empty($mts_options['homepage_twitter_access_token_secret']) || empty($mts_options['homepage_twitter_username'])){
            echo '<div class="mts_recent_tweets tweets section-content">'.__('The section is not configured correctly', 'mythemeshop').'</div></div></div>'; return;  }
        ...
        if ( file_exists(dirname(__FILE__).'/../functions/twitteroauth.php') ) {
            require_once(dirname(__FILE__).'/../functions/twitteroauth.php');
        } else {
            echo '<div class="mts_recent_tweets tweets section-content">'.__('Couldn\'t find twitteroauth.php!', 'mythemeshop').'</div></div></div>'; return;
        }                                                       
        ...
        $tweets = $connection->get("https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=".$mts_options['homepage_twitter_username']."&count=".$mts_options['homepage_twitter_tweet_count']);
        if ( empty( $tweets ) ) { echo '<div class="mts_recent_tweets tweets section-content">';_e( 'Couldn\'t retrieve tweets! Wrong username?', 'mythemeshop' ); echo '</div></div></div>'; return; }
        if(!empty($tweets->errors)){
            echo '<div class="mts_recent_tweets tweets section-content">';
