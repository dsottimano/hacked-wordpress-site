...
define( 'MTS_THEME_VERSION', '1.1.11' );
        ...
        if ( !empty( $mts_options['mts_custom_sidebars'] ) && is_array( $mts_options['mts_custom_sidebars'] )) {
            foreach( $mts_options['mts_custom_sidebars'] as $sidebar ) {
                if ( !empty( $sidebar['mts_custom_sidebar_id'] ) && !empty( $sidebar['mts_custom_sidebar_id'] ) && $sidebar['mts_custom_sidebar_id'] != 'sidebar-' ) {
                    register_sidebar( array( 'name' => ''.$sidebar['mts_custom_sidebar_name'].'', 'id' => ''.sanitize_title( strtolower( $sidebar['mts_custom_sidebar_id'] )).'', 'before_widget' => '<div id="%1$s" class="widget panel %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3 class="widget-title">', 'after_title' => '</h3>' ));
                }
            }
        }
    }
...
function mts_correct_homepage_sections_import( $item, $key, $data ) {
    ...
    if ( !in_array( $key, array( 'mts_custom_slider', 'mts_homepage_service', 'mts_homepage_clients', 'mts_homepage_team', 'mts_homepage_quotes' ) ) ) return $item;
    ...
    } elseif ( 'mts_homepage_service' === $key ) {

        foreach( $item as $i => $image ) {

            $id = $image['mts_homepage_service_image'];

            if ( is_numeric( $id ) ) {

                if ( array_key_exists( $id, $data['posts'] ) ) {

                    $new_item[ $i ]['mts_homepage_service_image'] = $data['posts'][ $id ];
                }
                ...
                if ( array_key_exists( $id, $data['image_urls'] ) ) {
                    ...
                    $new_item[ $i ]['mts_homepage_service_image'] = $data['image_urls'][ $id ];
                }
            }
        }

    } elseif ( 'mts_homepage_clients' === $key ) {

        foreach( $item as $i => $image ) {

            $id = $image['mts_homepage_client_image'];

            if ( is_numeric( $id ) ) {

                if ( array_key_exists( $id, $data['posts'] ) ) {

                    $new_item[ $i ]['mts_homepage_client_image'] = $data['posts'][ $id ];
                }
                ...
                if ( array_key_exists( $id, $data['image_urls'] ) ) {
                    ...
                    $new_item[ $i ]['mts_homepage_client_image'] = $data['image_urls'][ $id ];
                }
            }
        }

    } elseif ( 'mts_homepage_team' === $key ) {

        foreach( $item as $i => $image ) {

            $id = $image['mts_homepage_team_image'];

            if ( is_numeric( $id ) ) {

                if ( array_key_exists( $id, $data['posts'] ) ) {

                    $new_item[ $i ]['mts_homepage_team_image'] = $data['posts'][ $id ];
                }
                ...
                if ( array_key_exists( $id, $data['image_urls'] ) ) {
                    ...
                    $new_item[ $i ]['mts_homepage_team_image'] = $data['image_urls'][ $id ];
                }
            }
        }

    } else { // mts_homepage_quotes

        foreach( $item as $i => $image ) {

            $id = $image['mts_homepage_quote_image'];

            if ( is_numeric( $id ) ) {

                if ( array_key_exists( $id, $data['posts'] ) ) {

                    $new_item[ $i ]['mts_homepage_quote_image'] = $data['posts'][ $id ];
                }
                ...
                if ( array_key_exists( $id, $data['image_urls'] ) ) {
                    ...
                    $new_item[ $i ]['mts_homepage_quote_image'] = $data['image_urls'][ $id ];
                }
            }
        }
    }
